...
export default function MedicalRecords() {
  ...
  const card = { background: 'var(--bg-card)', padding: '1rem 1.5rem', borderRadius: '10px', marginBottom: '1rem', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', display:'flex', justifyContent:'space-between', alignItems:'center' }
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap', marginBottom:'1.5rem' }}>
        {CATEGORIES.map(c => (
          <button key={c} onClick={() => setFilter(c)} style={{ padding:'0.4rem 1rem', borderRadius:'20px', border:'none', cursor:'pointer', fontSize:'0.85rem', background: filter === c ? '#4361ee' : 'var(--bg-subtle)', color: filter === c ? 'white' : 'var(--text-secondary)' }}>
            {c}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 && <p style={{ color: 'var(--text-muted)' }}>No records found.</p>}
