...
export default function Medications() {
  ...
  const card = { background: 'var(--bg-card)', padding: '1.2rem 1.5rem', borderRadius: '10px', marginBottom: '1rem', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto' }}>
      ...
      {filtered.length === 0 && <p style={{ color: 'var(--text-muted)' }}>No medications found.</p>}
        ...
        <div key={m._id} style={card}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
            <div>
              ...
              <p style={{ color: 'var(--text-muted)', margin:'0.2rem 0 0.8rem', fontSize:'0.9rem' }}>{m.dosage} · {m.frequency}</p>
            </div>
          ...
          <div style={{ display:'flex', gap:'0.75rem' }}>
            {['morning', 'afternoon', 'night'].map(time => (
              <button key={time} onClick={() => toggleTime(m._id, time, m[time])} style={{ padding:'0.4rem 1rem', borderRadius:'20px', border:'none', cursor:'pointer', fontSize:'0.85rem', background: m[time] ? '#4361ee' : 'var(--bg-subtle)', color: m[time] ? 'white' : 'var(--text-secondary)', transition:'all 0.15s' }}>
